...
# Board resetting, crashing and unstable
...
> [!TIP]
> `modprobe` isn't available on the buildroot, so `insmod` is the only way to load drivers, however it requires you to manually load drivers in the correct order of dependency, check `/oem/usr/ko/insmod_ko.sh` to see which order they must be loaded

# SC3336 Camera
<img width="817" height="554" alt="image" src="https://github.com/user-attachments/assets/9f32f8a9-0d06-475d-834f-0927b3009869" />
...
```
> [!IMPORTANT]
> Make sure to leave [24MB ram dedicated](https://github.com/themrleon/luckfox-pico-mini-b/tree/main?tab=readme-ov-file#why-there-isnt-64mb-ram-available-) to the camera

### Camera stream server (rkipc)
...
> [!TIP]
> The `rkipc` binary as well as other shipped tools/binaries are all available on the environment/path so can be run/accessed from anywhere

> [!TIP]
> To quick stop the stream server run `killall rkipc`

> [!WARNING]
> Unfortunately right aftert boot the camera will start spamming the FIQ/debugger interface, so if you are using that things will be unbearable it will overwrite any thing you type, the camera also is detecting movements generating messages all the time, in order to silence all that you'll have to generate another OS image after removing all `LOG_INFO` from `<sdk path>/project/app/rkipc/rkipc/src/rv1103_ipc/video/video.c`

### Camera stream server settings (rkipc.ini)
...
> [!TIP]
> Run `simple_vi_bind_venc --help` to see all options available

> [!NOTE]
> I had no problems recording h265 at max resolution despite my ~20MBs SD card

> [!TIP]
> For stream video recording, VLC supports recording (on desktop version) 
...
> [!IMPORTANT]
> Changing the exposure manually may work but eventually it will be overwritten by the stream server, to stop that from happening you have to edit `rkipc.ini` and disable auto exposure setting 
